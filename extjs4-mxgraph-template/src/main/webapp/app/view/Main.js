Ext.define('MyApp.view.Main', {
	...
	renderGraph : function() {
		...
		if (!mxClient.isBrowserSupported()) {
			...
			var container = document
					.getElementById(mainLayout.graphContainer.id);
			// Creates the graph inside the given container
			var editor = new mxEditor();
			var graph = editor.graph;
			var scale = graph.view.scale;
			var bounds = graph.getGraphBounds();
			graph.view.setTranslate(10 - bounds.x / scale, 10 - bounds.y
							/ scale);
			...
			try {
				var v1 = graph.insertVertex(parent, null, UserObject(), 20, 20,
						80, 30);
	...
	getContainerTbarButtons : function(mainLayout) {
		return [{
					...
					handler : function(button) {
						var selectedCell = mainLayout.editor.graph
								.getSelectionCell();
						if (Ext.isEmpty(selectedCell)
								|| selectedCell.vertex !== true) {
							Ext.Msg.alert('warning', 'Please select a vertex');
							return;
						}
						var parent = mainLayout.graph.getDefaultParent();
						Ext.Msg.prompt('Name', 'Please enter vertex name ',
								function(btn, text) {
									if (btn == 'ok') {
										try {
											var vertex = mainLayout.graph
													.insertVertex(parent, null,
															text, 20, 20, 80,
															30);
											var edge = mainLayout.graph
													.insertEdge(parent, null,
															'', selectedCell,
															vertex);
										} finally {
											mainLayout.graph.getModel()
													.endUpdate();
											mainLayout
													.executeLayout(mainLayout.editor.graph);
										}
									}
								});
					...
					handler : function(button) {
						...
						var form = Ext.create('Ext.form.Panel', {
							...
							buttons : [{
								...
								text : 'Upload',
								handler : function(button) {
									var file = button.up('form').items.items[0]
											.getEl().down('input[type=file]').dom.files[0];
